...
export default defineComponent({
  setup() {
    ...
    const switchTrigger = (event) => {
      let target = event.target;
      if (!target.classList.contains('switch')) target = target.parentElement
      const switchBtn = target.querySelector('.switch-btn')
      if (switchBtn) {
        switchBtn.click()
      }
    };
...
<template>
  <div class="overlay" v-show="visible">
    <div
      draggable="false"
      :class="['popup', asideClose ? 'popup-close' : 'popup-open']"
    >
      <!-- 侧边按钮信息 -->
        ...
        <div class="action" v-for="action in list" :key="action.id">
          ...
          <div class="action-item" v-if="action.isInput">
            ...
            <div class="action-item-value switch" @click="switchTrigger">
              <input
                class="block w-40 rounded-md border-0 py-1.5 pl-7 pr-7 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6 switch-btn"
                type="checkbox"
                v-model="action.enter"
                placeholder="switch"
              />
